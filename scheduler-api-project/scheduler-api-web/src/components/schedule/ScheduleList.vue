          ...
          <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
              <table class="min-w-full">
                  ...
                  <tbody class="bg-white">
                        <!-- <ScheduleItem 
                        v-for="job in this.$store.state.Schedule.jobList" :key="job.labelId" :item="job">
                        </ScheduleItem> -->
                      <tr 
                      @click="goToJobDetail(job.labelId)" 
                      v-for="job in this.jobs" :key="job.labelId"
                      class="hover:bg-gray-50 focus:outline-none cursor-pointer">
                        <td class="px-6 py-4 text-sm leading-5 text-gray-500 border-b border-gray-200 whitespace-nowrap">
                            {{ job.name }} ({{ job.username }})
                        </td>
